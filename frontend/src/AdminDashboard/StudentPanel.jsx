  ...
  const acceptUser = async ({ student, id }) => {
    try {
      const hash = CryptoJS.SHA256(
        student.attributes.email + student.attributes.username
  ...
  return (
    <div className="bg-gray-200 w-full min-h-screen">
      <div className="container mx-auto py-8">
        <h1 className="text-3xl font-bold mb-4">Student Panel</h1>
        <div className="grid grid-cols-1 gap-4">
          <div className="bg-blue-300 p-4 rounded-lg flex justify-between items-center">
            <div className="flex items-center justify-between w-1/2">
              <h1 className="font-bold mr-2">Student Name</h1>
          ...
          {students &&
            students.map((student) => {
              if(student.attributes.isFaculty !== true)
              return (
                <div
                  key={student.id}
                ...
                >
                  <div className="flex items-center justify-between w-1/2">
                    <p className="font-bold mr-2">
                      ...
                      {
                        student.attributes.course.data.attributes.courseName 
                      } {" "} ( {student.attributes.branch.data.attributes.name}  )
                    </p>
                  </div>
